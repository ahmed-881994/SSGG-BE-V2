...
class AuditMiddleware:
    async def __call__(self, request: Request, call_next):
        ...
        user_id, member_id = await self.get_user_id_from_request(request)
        ...
        try:
            ...
            response_data = await self.capture_response_data(response)
    ...
    async def get_user_id_from_request(self, request: Request) -> tuple[Optional[int], Optional[str]]:
        """Extract user ID from request - adjust based on your auth implementation"""
        try:
    ...
    async def capture_request_data(self, request: Request) -> str:
        ...
        try:
            ...
            if payload_raw:
                try:
                    ...
                    parsed_payload = self.mask_sensitive_response_data(parsed_payload)
                    payload_to_store = parsed_payload
                except json.JSONDecodeError:
                    # If not valid JSON, check for sensitive data in raw text
                    payload_to_store = self.mask_sensitive_text(payload_raw)
            else:
    ...
    def mask_sensitive_json_data(self, data):
        ...
        if isinstance(data, dict):
            ...
            for key, value in data.items():
                if key.lower() in ['password', 'passwd', 'pwd', 'oldpassword', 'newpassword', 'secret', 'token', 'key', 'auth']:
                    ...
                    masked_data[key] = self.mask_sensitive_json_data(value)
                else:
                    masked_data[key] = value
            return masked_data
        elif isinstance(data, list):
            return [self.mask_sensitive_json_data(item) for item in data]
        else:
            return data


    def mask_sensitive_form_data(self, body_str: str) -> str:
    ...
    def mask_sensitive_response_data(self, data):
        ...
        if isinstance(data, dict):
            ...
            for key, value in data.items():
                ...
                if key.lower() in ['access_token', 'refresh_token', 'token', 'password', 'secret', 'key']:
                    ...
                    masked_data[key] = self.mask_sensitive_response_data(value)
                else:
                    masked_data[key] = value
            return masked_data
        elif isinstance(data, list):
            return [self.mask_sensitive_response_data(item) for item in data]
        else:
            return data
    ...
    async def save_audit_record(self,
