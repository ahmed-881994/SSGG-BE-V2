...
def update_health_metrics(report: dict) -> None:
    ...
    # Overall
    HEALTH_OVERALL_UP.set(_status(report.get("status", "unhealthy")))
    HEALTH_CHECK_DURATION_MS.set(report.get("response_time_ms", 0))
    summary = report.get("summary", {})
    HEALTH_SERVICES_HEALTHY.set(summary.get("healthy_services", 0))
    HEALTH_SERVICES_WARNING.set(summary.get("warning_services", 0))
    HEALTH_SERVICES_UNHEALTHY.set(summary.get("unhealthy_services", 0))
    ...
    if db:
        HEALTH_DB_UP.set(_status(db.get("status", "unhealthy")))
        HEALTH_DB_RESPONSE_MS.set(db.get("performance", {}).get("response_time_ms", 0))

    # Connection pool
    pool = services.get("connection_pool", {})
    if pool:
        HEALTH_POOL_UP.set(_status(pool.get("status", "unhealthy")))
        HEALTH_POOL_RESPONSE_MS.set(pool.get("response_time_ms", 0))
        pool_cfg = pool.get("pool_configuration", {})
        pool_cur = pool.get("current_metrics", {})
        HEALTH_POOL_SIZE.set(pool_cfg.get("max_pool_size", 0))
        HEALTH_POOL_ACTIVE.set(pool_cur.get("active_connections", 0))
        HEALTH_POOL_AVAILABLE.set(pool_cur.get("available_connections", 0))
        HEALTH_POOL_OVERFLOW.set(pool_cur.get("overflow_connections", 0))
        HEALTH_POOL_UTILIZATION.set(pool_cur.get("utilization_percentage", 0))

    # Database schema
    schema = services.get("database_schema", {})
    if schema:
        HEALTH_SCHEMA_UP.set(_status(schema.get("status", "unhealthy")))
        HEALTH_SCHEMA_RESPONSE_MS.set(schema.get("response_time_ms", 0))
        HEALTH_SCHEMA_REQUIRED_TABLES.set(schema.get("required_tables_count", 0))
        HEALTH_SCHEMA_EXISTING_TABLES.set(schema.get("existing_table_count", 0))
        missing = schema.get("missing_tables", [])
        HEALTH_SCHEMA_MISSING_TABLES.set(len(missing) if isinstance(missing, list) else 0)

    # Environment
    env = services.get("environment", {})
    if env:
        HEALTH_ENV_UP.set(_status(env.get("status", "unhealthy")))
        HEALTH_ENV_RESPONSE_MS.set(env.get("response_time_ms", 0))

    # Redis
    redis_data = services.get("redis_token_blacklist", {})
    if redis_data:
        HEALTH_REDIS_UP.set(_status(redis_data.get("status", "unhealthy")))
        HEALTH_REDIS_RESPONSE_MS.set(redis_data.get("response_time_ms", 0))
        perf = redis_data.get("performance_metrics", {})
        HEALTH_REDIS_CONNECTED_CLIENTS.set(perf.get("connected_clients", 0))
        HEALTH_REDIS_KEYSPACE_HITS.set(perf.get("keyspace_hits", 0))
        HEALTH_REDIS_KEYSPACE_MISSES.set(perf.get("keyspace_misses", 0))
